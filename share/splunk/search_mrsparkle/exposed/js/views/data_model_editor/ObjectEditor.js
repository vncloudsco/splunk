        ...
        )
{
    return BaseView.extend({
        ...
        render: function() {
            if (this.model.objectModel.get("indexScopeWarning") === true) {
                this.options.flashMessagesView.flashMsgHelper.addGeneralMessage("index_scope_warning",
                    {
                        type: splunkDUtils.WARNING,
                        html: _("This object has no explicit index constraint. Consider adding one for better performance.").t()
                    });
            } else {
                this.options.flashMessagesView.flashMsgHelper.removeGeneralMessage("index_scope_warning");
            }
            ...
            // Only non-accelerated Data Models with no source GUID can be edited.
