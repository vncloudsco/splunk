    ...
    buildModuleInstanceList: function() {
        ...
        for (var i=0, lim=moduleContainers.length; i<lim; i++) {
            ...
            if (subclass in Splunk.Module) {
                ...
                subclass);
                Splunk.util.redirect_to('error/render_error_page', {
                    'message' : _('Looks like this view is using Advanced XML, which has been removed from Splunk Enterprise. Please contact your Splunk administrator.'),
                    'status' : '', // Kept empty as setting it to 404 overrides above message to hardcoded page not found message
                    'learnmore': 'learnmore.python3migration.advancedxml'
                });
                break; // Break iterating on all modules when any one module is not found
            }
        }
